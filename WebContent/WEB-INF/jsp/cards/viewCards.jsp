		        ...
		        success: function(res) {
		        	 if(res.validated)
		        	 {
		                ...
		                $.each(res.searchedCards, function (i, item) {
		                    trHTML += '<tr><td>' + (i+1) + '</td><td>' + item.cardHoldername + '</td><td>' + item.cardNumber +'</td><td>' + item.expDate +'</td><td>' + 'Edit' + '</td></tr>';
		                });
...
<body>
<div id="customer">
        <h3 class="text-info panel">Welcome ${pageContext.request.userPrincipal.name}</h3>
